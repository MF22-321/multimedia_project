...
def landmarks_to_bbox(face_lms, w, h, pad=0.20):
    ...
    xmin = max(0.0, xmin - pad * bw)
    ymin = max(0.0, ymin - pad * bh)
    xmax = min(1.0, xmax + pad * bw)
    ymax = min(1.0, ymax + pad * bh)
...
class LandmarkerFaceCropper:
    def __init__(self):
        ...
        print("[LANDMARKER] exists =", LANDMARKER_PATH.exists())
        ...
        # read model as bytes to avoid Windows path resolution issue in MediaPipe
        model_bytes = LANDMARKER_PATH.read_bytes()
